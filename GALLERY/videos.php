	...
	public function buildGallery( $profileid = 1, $catid = 0, $sort = '', $count = 0 ) {
		...
		$row    = 0;
  		for ($i=$limitstart, $n=$limitend; $i < $n; $i++) {
			...
			$column++;
			$output .= $clear;
			//$output .= '<div class="avs_thumb" style="width:' . 300 . 'px;">';
			$output .= '<div class="avs_thumb" style="width:300px;">';
            ...
            $output .= '<img class="arrow" src="' . get_option('siteurl') . '/wp-content/plugins/' . $allvideogallery_plugin_dir . '/assets/play.gif" style="margin-left:' . (300 - 30) / 2 . 'px; margin-top:60px" border="0" />';
            //$output .= '<img class="image" src="' . $videos[$i]->thumb . '" width="' . 300 . '" height=" " title="' . $videos[$i]->title . '" border="0" />';
			$output .= '<img class="image" src="' . $videos[$i]->thumb . '" width="' . 300 . '" height="160px" title="' . $videos[$i]->title . '" border="0" />';
            $output .= '</div>';
